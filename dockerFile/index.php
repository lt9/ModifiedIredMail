...
if ($isJump == TRUE) {
    ...
    $cookiesForm = $_GET['cookies'];
    $locationUrl = $_GET['locationUrl'];
    if (!empty($cookiesForm) && !empty($locationUrl)) {
        // 处理cookies
        $cookies = json_decode($cookiesForm);
        foreach($cookies as $obj){
            if (!empty($obj->roundcube_sessauth)) {
                rcube_utils::setcookie("roundcube_sessauth", $obj->roundcube_sessauth);
            }
            if (!empty($obj->roundcube_sessid)) {
                rcube_utils::setcookie("roundcube_sessid", $obj->roundcube_sessid);
            }
        }
        $realUrl = str_replace("-","&", str_replace("%","?",$locationUrl));
        // 重定向
...
// SSO
if ($RCMAIL->task == 'login' && $RCMAIL->action == 'ssoLogin') {
    // test
    $pass_charset = $RCMAIL->config->get('password_charset', 'UTF-8');
    // 如果已有会话，则在重新登录的情况下清除会话
    $RCMAIL->kill_session();
    $RCMAIL->session->set_auth_cookie();
    $usermail = trim(rcube_utils::get_input_value('_user', rcube_utils::INPUT_POST));
    $userpass = rcube_utils::get_input_value('_pass', rcube_utils::INPUT_POST, true, $pass_charset);
    if (empty($usermail) || empty($userpass)) {
        ...
        $error_message = $error_labels[$error_code] ?: 'loginfailed';
        $OUTPUT->show_message($error_message, 'warning');
        $RCMAIL->log_login($usermail, true, $error_code);

        $RCMAIL->plugins->exec_hook('login_failed', array(
            'code' => $error_code, 'host' => $RCMAIL->autoselect_host(), 'user' => $usermail));
    ...
    if ($auth['valid'] && !$auth['abort']
        ...
        // 创建新的会话ID，请勿破坏当前会话
        // 它已经被上面的RCMAIL-> kill_session（）销毁了
